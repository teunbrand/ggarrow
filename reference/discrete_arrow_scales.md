...
- values:
  ...
  - An unnested `<list>`, possibly mixed `<list>`, containing any of the
    ...
    - A `<function>` that when called without any arguments produces a
      2-column `<matrix>` that can be used as an arrow.

    - A 2-column `<matrix>` giving a polygon to use as an arrow.

  - `NULL`, which defaults to a built-in palette with a maximum of 3
    arrows.
