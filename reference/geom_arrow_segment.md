...
(xend, yend). In contrast to
[`geom_segment()`](https://ggplot2.tidyverse.org/reference/geom_segment.html),
the `xend` and `yend` aesthetics default to `x` and `y` respectively, so
...
`geom_arrow_segment()` understands the following aesthetics. Required
...
| • | **[`xend`](https://ggplot2.tidyverse.org/reference/aes_position.html) *or* [`yend`](https://ggplot2.tidyverse.org/reference/aes_position.html)** |  |
...
The `linewidth_fins` and `linewidth_head` inherit from `linewidth`. They
can be used to separately control the start- and end-width.
